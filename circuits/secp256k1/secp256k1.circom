...
include "../bigint/bigint_4x64_mult.circom";
...
template Secp256k1AddUnequal() {
    ...
    var tmp[2][100] = secp256k1_addunequal_func(64, 4, x1, y1, x2, y2);
    for(var i = 0; i < 4;i++){
        out[0][i] <-- tmp[0][i];
        out[1][i] <-- tmp[1][i];
    }
...
template Secp256k1ScalarMult(n, k) {
    ...
    for (var i = k - 1; i >= 0; i--) {
        for (var j = n - 1; j >= 0; j--) {
            ...
            if (i < k - 1 || j < n - 1) {
                adders[n * i + j] = Secp256k1AddUnequal();
